 ...
 * @param[in]   p_nus       Nordic UART Service structure.
 * @param[in]   p_ble_evt   Event received from the BLE stack.
 * p_evt_write->data stores the received datas from BLE stack, p_evt_write->len is the length of received datas
 */
...
{  
    ...
    )
    {
        ...
        if (value == 0x001)   //judge if the service is added or not
        {
